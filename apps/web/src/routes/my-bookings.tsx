      ...
      {bookings.length === 0 ? (
        ...
        <div className="grid grid-cols-1 gap-4">
          {bookings.map((b) => (
            <div
              key={b.id}
              className="dashboard-card p-5 border border-[var(--line)] transition-all hover:border-indigo-500/30 flex flex-col md:flex-row gap-6 items-start md:items-center justify-between"
            >
              {/* Left Info */}
              <div className="flex items-center gap-5 w-full md:w-auto">
                <div
                  className={`p-4 rounded-2xl flex items-center justify-center ${b.status === "pending" ? "bg-amber-500/10 text-amber-500" : "bg-emerald-500/10 text-emerald-500"}`}
                >
                  {b.status === "pending" ? (
                    <Banknote size={24} />
                  ) : (
                ...
                <div>
                  ...
                  <div className="flex items-center gap-4 text-sm text-[var(--sea-ink-soft)]">
                    <span className="flex items-center gap-1">
                      <MapPin size={14} /> {b.slot.floor.parkingLot.name} (Fl.{" "}
                      {b.slot.floor.level})
                    </span>
                    <span className="flex items-center gap-1">
