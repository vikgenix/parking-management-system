  ...
  return (
    <aside className="hidden w-64 border-r border-[var(--line)] bg-[var(--surface-strong)] flex-col pt-6 backdrop-blur-xl lg:flex sticky top-0 h-screen z-50">
      <div className="px-6 mb-8 flex items-center gap-3">
         ...
         <h1 className="text-xl font-bold tracking-tight text-[var(--sea-ink)]">ParkAdmin<span className="text-indigo-500">.</span></h1>
      </div>
      
      <nav className="flex-1 px-4 space-y-1.5 overflow-y-auto">
         <div className="text-xs font-semibold text-[var(--sea-ink-soft)] uppercase tracking-wider mb-2 px-3 mt-4">Overview</div>
         <SidebarLink to="/" icon={<LayoutDashboard size={20} />} label="Dashboard" />
         
         <div className="text-xs font-semibold text-[var(--sea-ink-soft)] uppercase tracking-wider mb-2 px-3 mt-8">Operations</div>
         <SidebarLink to="/" icon={<CalendarCheck size={20} />} label="Live Bookings" />
         <SidebarLink to="/" icon={<Car size={20} />} label="Vehicles" />
         <SidebarLink to="/" icon={<Map size={20} />} label="Floors & Slots" />
         
         <div className="text-xs font-semibold text-[var(--sea-ink-soft)] uppercase tracking-wider mb-2 px-3 mt-8">Finance</div>
         <SidebarLink to="/" icon={<CreditCard size={20} />} label="Payments & Revenue" />
      </nav>
      
      <div className="p-4 border-t border-[var(--line)] space-y-1.5">
         <SidebarLink to="/" icon={<Settings size={20} />} label="System Settings" />
...
function SidebarLink({ to, icon, label }: { to: string; icon: React.ReactNode; label: string }) {
  return (
    <Link 
      ...
      activeProps={{ className: 'bg-indigo-500/10 text-indigo-500 font-semibold shadow-sm' }}
    >
